    ...
    <Card>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="text-center">
          ...
          <div className="text-xs text-muted-foreground">Total golpes</div>
        </div>
        <div className="text-center">
      ...
      <div className="grid grid-cols-2 gap-2 mb-4">
        <div className="bg-purple-500/10 rounded-md p-2 border border-purple-500/20">
          <div className="text-xs text-muted-foreground">Con Pared</div>
          <div className="text-sm font-bold text-purple-400">{stats.wallVsNoWall.withWall.winPct}%</div>
          <div className="text-xs text-muted-foreground">{stats.wallVsNoWall.withWall.wins}/{stats.wallVsNoWall.withWall.total}</div>
        </div>
        <div className="bg-blue-500/10 rounded-md p-2 border border-blue-500/20">
          <div className="text-xs text-muted-foreground">Sin Pared</div>
          <div className="text-sm font-bold text-blue-400">{stats.wallVsNoWall.withoutWall.winPct}%</div>
          <div className="text-xs text-muted-foreground">{stats.wallVsNoWall.withoutWall.wins}/{stats.wallVsNoWall.withoutWall.total}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="bg-background/50 rounded p-2">
          <div className="text-xs text-muted-foreground">Vidrio (baja)</div>
          <div className="text-sm font-bold">{stats.wallByHeight.baja.count} golpes</div>
          <div className="text-xs text-muted-foreground">{stats.wallByHeight.baja.winRate}% win rate</div>
        </div>
        <div className="bg-background/50 rounded p-2">
          <div className="text-xs text-muted-foreground">Reja (alta)</div>
          <div className="text-sm font-bold">{stats.wallByHeight.alta.count} golpes</div>
          <div className="text-xs text-muted-foreground">{stats.wallByHeight.alta.winRate}% win rate</div>
        </div>
      </div>
      ...
      {stats.mostUsedWalls.length > 0 && (
        <div>
          <div className="text-xs text-muted-foreground mb-1">Paredes mas usadas</div>
